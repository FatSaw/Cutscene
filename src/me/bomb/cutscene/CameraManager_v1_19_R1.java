...
class CameraManager_v1_19_R1 extends CameraManager {
	...
	protected CameraManager_v1_19_R1() {
		new BukkitRunnable(){
		    @Override
		    public void run(){
                		...
                		case b :
                			...
                			for (int j = 0; j < i; ++j) {
                				...
                				if(player.getUniqueId().equals(uuid)) {
                					...
                					gamemodes.put(uuid, packetdataserializer.readByte());
                				}
                			}
	...
	private void spawnCamera(EntityPlayer entityplayer, RouteProvider route, EntityLiving cameraentity) {
		eas.put(entityplayer.cp(), cameraentity);
		pos.put(entityplayer.cp(), route);
	...
	private void despawnCamera(EntityPlayer entityplayer, EntityLiving cameraentity) {
		eas.remove(entityplayer.cp());
		pos.remove(entityplayer.cp());
		PlayerConnection connection = entityplayer.b;
